...
class Trainer:
    def __init__(
        ...
        if dist_utils.is_main_process():
            self.tb_writer, _ = get_tb_writer(log_args.logdir, log_args.filename)  # 写进tensorboard
        dist_utils.barrier()
        ...
        # print("准备进入resume")
        # self.resume("/nfs/lhl/Tree-ViT/vit_mutual/tasks/best.pth")
        # self.resume("/nfs/lhl/Tree-ViT/run/cifar10/sparse_vit/small-global-sparse-vit-small/ckpt/best.pth")
        self.resume("/nfs/lhl/Tree-ViT/run/imagenet/pit/sparse-pit-tiny/ckpt/best.pth")
        # print("resume好了")
        # exit(0)
        self.logger.info("Start training for %d epochs", self.train_cfg["train_epochs"] - self.start_epoch)

    def resume(self, resume_fp: str = "best.pth"):
        """
    ...
    def train_iter(self, x: Tensor, targets: List[Dict[str, Any]]):
        ...
        with amp.autocast(enabled=self.amp):  # 作用域之内的使用半精度加速训练
            output = self.model(x)
    ...
    def __call__(self):  # 使得类实例对象可以像调用普通函数那样以对象名的形式使用
