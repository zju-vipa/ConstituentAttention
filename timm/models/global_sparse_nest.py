...
class Attention(nn.Module):
    ...
    def __init__(self, dim, seq_length, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        ...
        self.linear_key = nn.Linear(dim, dim, bias=qkv_bias)
        self.linear_query = nn.Linear(dim, dim, bias=qkv_bias)
        self.linear_key_column = nn.Linear(dim, dim, bias=qkv_bias)
        self.linear_query_column = nn.Linear(dim, dim, bias=qkv_bias)
        self.a = torch.from_numpy(np.diag(np.ones(seq_length - 1, dtype=np.int32), 1)).cuda()
        self.b = torch.from_numpy(np.diag(np.ones(seq_length, dtype=np.int32), 0)).cuda()
        self.tri_matrix = torch.from_numpy(np.triu(np.ones([seq_length, seq_length], dtype=np.float32), 0)).cuda()
        self.mapping_table = cal_idx(seq_length, int(math.sqrt(seq_length)))
        self.embed_dim = dim
    ...
    def forward(self, x, prior):
        ...
        context_column = x[:, :, self.mapping_table, :]

        qkv = self.qkv(x).reshape(B, T, N, 3, self.num_heads, C // self.num_heads).permute(3, 0, 4, 1, 2, 5)
        q, k, v = qkv.unbind(0)  # make torchscript happy (cannot use tensor as tuple)

        key = self.linear_key(x)  # (B, T, N, C)
        # print("key.shape: ", key.shape)
        query = self.linear_query(x)  # (B, T, N, C)
        # print("query.shape: ", query.shape)
        key_column = self.linear_key_column(context_column)
        # print("key.shape: ", key.shape)
        query_column = self.linear_query_column(context_column)
        scores = torch.matmul(query, key.transpose(-2, -1)) / self.embed_dim  # (B, T, N, N)
        ...
        g_attn = g_attn + g_attn.transpose(-2, -1) + neibor_attn.masked_fill(self.b == 0, 1e-9)  # (B, T, N, N)
...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, seq_length, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, seq_length=seq_length, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, group_prob):
        x = self.norm1(x)
        tmp, g_attn = self.attn(x, group_prob)
...
class NestLevel(nn.Module):
    ...
    def __init__(
        ...
        self.transformer_encoder = nn.Sequential(*[
            TransformerLayer(
                dim=embed_dim, num_heads=num_heads, seq_length=seq_length, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
    ...
    def forward(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for encoder in self.transformer_encoder:
                x, group_prob = encoder(x, group_prob)  # (B, T, N, C')
...
class Nest(nn.Module):
    ...
    def __init__(
            ...
            num_heads=(4, 8, 16), depths=(2, 2, 20), num_classes=100, mlp_ratio=4., qkv_bias=True,
...
def nest_tiny(pretrained=False, **kwargs):
    """ Nest-T @ 224x224
    """
    model_kwargs = dict(embed_dims=(96, 192, 384), num_heads=(3, 6, 12), depths=(2, 2, 8),
                        num_classes=100, drop_path_rate=0.1, **kwargs)
