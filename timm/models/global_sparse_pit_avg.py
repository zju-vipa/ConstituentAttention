...
from .layers import trunc_normal_, to_2tuple, create_conv2d, create_pool2d, to_ntuple
from .registry import register_model
from .global_sparse_vision_transformer import Block
# from .small_global_sparse_vision_transformer import Block
...
class Transformer(nn.Module):
    def __init__(
            self, base_dim, depth, seq_length, heads, mlp_ratio, pool=None, drop_rate=.0, attn_drop_rate=.0,
            drop_path_prob=None):
...
class ConvHeadPooling(nn.Module):
    def __init__(self, in_feature, out_feature, stride, pad_type=''):
        super(ConvHeadPooling, self).__init__()
        # 更换了下采样的方式！
        norm_layer = partial(nn.LayerNorm, eps=1e-6)
        self.conv = create_conv2d(in_feature, out_feature, kernel_size=3, padding=pad_type, bias=True)
    ...
    def __init__(
        ...
        for stage in range(len(depth)):
            ...
            transformers += [Transformer(
                base_dims[stage], depth[stage], height * width, heads[stage], mlp_ratio, pool=pool,
