...
class Origin_TransformerLayer(nn.Module):
    """
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., drop_path=0.,
        ...
        self.attn = Origin_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, group_prob):
        y = self.norm1(x)
        tmp, g_attn = self.attn(y, group_prob)
...
class Origin_NestLevel(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(self.num_blocks)):
            if i != len(self.num_blocks) - 1:
                dim = embed_dims[i]
                levels.append(Origin_NestLevel(
