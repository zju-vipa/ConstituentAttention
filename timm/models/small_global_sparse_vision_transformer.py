...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, seq_length=196, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        ...
        self.linear_key = nn.Linear(dim, head_dim, bias=qkv_bias)
        self.linear_query = nn.Linear(dim, head_dim, bias=qkv_bias)
        self.linear_key_column = nn.Linear(dim, head_dim, bias=qkv_bias)
        self.linear_query_column = nn.Linear(dim, head_dim, bias=qkv_bias)
    ...
    def forward(self, x, prior):
        ...
        scores = torch.matmul(query, key.transpose(-2, -1)) * self.scale # (B, T, N, N)
        scores_column = torch.matmul(query_column, key_column.transpose(-2, -1)) * self.scale
        ...
        g_attn_column = g_attn_column + g_attn_column.transpose(-2, -1) + neibor_attn_column.masked_fill(self.b == 0,
                                                                                                         1e-9)  # 文中的C
...
class Block(nn.Module):
    ...
    def __init__(
            self, dim, num_heads, seq_length, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, seq_length=seq_length, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                dim=embed_dim, num_heads=num_heads, seq_length=embed_len, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for block in self.blocks:
                x, group_prob = block(x, group_prob)
        x = self.norm(x)
        return x
