...
def visualize_grid_attention_v2(img_path, save_path, attention_mask, ratio=1, cmap="jet", save_image=False,
                             save_original_image=False, quality=200, cnt=2):
    """
    ...
    if save_image:
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        img_name = img_path.split('/')[-1].split('.')[0] + "_with_attention_layer" + str(cnt) + ".jpg"
...
def visualize_grid_attention_v3(img_path, save_path, attention_mask, ratio=1, cmap="jet", save_image=False,
                             save_original_image=False, quality=200, cnt=2, prefix="True"):
    """
    img_path:   image file path to load
    save_path:  image file path to save
    attention_mask:  2-D attention map with np.array type, e.g, (h, w) or (w, h)
    ...
    if save_image:
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        img_name = img_path.split('/')[-1].split('.')[0] + "_with_attention_layer" + str(cnt) + ".jpg"
        img_with_attention_save_path = os.path.join(save_path, prefix+img_name)
